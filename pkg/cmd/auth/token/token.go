...
func requestAutomicVaultApprovalForToken(hostname, username string) error {
	return automicvault.RequestApproval(newAutomicVaultApprovalRequest(hostname, username))
}

func newAutomicVaultApprovalRequest(hostname, username string) automicvault.ApprovalRequest {
	args := []string{"--hostname", hostname}
	if username != "" {
		args = append(args, "--user", username)
	}

	return automicvault.NewApprovalRequest("gh auth token", args)
}
